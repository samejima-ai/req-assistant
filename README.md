...
### 🎙️ Multi-LLM Selection: モデルを自由に使い分ける
  
**Vercel AI SDK** を統合。Google (Gemini 3.1), OpenAI (GPT-5.4), Anthropic (Claude 4.6) の中から、処理工程（インテント分析、設計図生成、レビュー等）ごとに最適なモデルを選択できます。非エンジニアでも使いやすい「高速・標準・思考」のティア設定ガイド付き。
...
- **Mermaid 図の双方向編集**: クリックでインライン編集が可能。

### 📱 Mobile-First Design: 外出先でも設計を止めない

モバイル環境に完全対応。
- **サイバー風 FAB (Floating Action Button)**: 設定やエクスポート機能を、画面 **右上** に集約。
- **独立した凡例・エッジ選択ボタン**: 凡例や接続種別設定をキャンバス **右下** のアイコンボタンに統合。ポップオーバー形式で開閉可能です。
- **レスポンシブ・ツールバー**: モバイルではアイコンのみの円形ボタンに自動切り替え。
...
| 変数名 | 説明 | デフォルト |
| --- | --- | --- |
| `VITE_GOOGLE_API_KEY` | Google Gemini API キー (VITE_GEMINI_API_KEYも可) | — |
